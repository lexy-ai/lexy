...
class TestIndexClient:
    ...
    def test_update_index(self, lx_client):
        test_index = lx_client.update_index(
            "test_index", description="Test Index Updated"
        )
        assert test_index.index_id == "test_index"
    ...
    async def test_aupdate_index(self, lx_async_client):
        test_index = await lx_async_client.index.aupdate_index(
            "test_index", description="Test Index Async Update"
        )
        assert test_index.index_id == "test_index"
    ...
    async def test_alist_index_records(self, lx_async_client):
        records = await lx_async_client.index.alist_index_records(
            index_id="default_text_embeddings"
        )
        assert len(records) >= 0
    ...
    def test_query_return_nonexistent_index_field(
    ...
    def test_query_return_nonexistent_document_field(
        self, lx_client, celery_app, celery_worker
    ):
        with pytest.raises(LexyAPIError) as exc_info:
            lx_client.index.query_index(
        ...
        assert (
            ...
            == '{"detail":"Field \'not_a_document_field\' not found in document"}'
        )
