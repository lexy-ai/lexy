...
async def get_collections(
    collection_name: str | None = None, session: AsyncSession = Depends(get_session)
...
async def create_collection(
    ...
    if existing_collection:
        raise HTTPException(
            status_code=400, detail="Collection with that name already exists"
        )
...
async def delete_collection_by_name(
    ...
    if delete_documents is True:
        ...
        statement = select(
            exists().where(Document.collection_id == collection_id)
...
async def update_collection(
    ...
    db_collection = await crud.get_collection_by_id(
        session=session, collection_id=collection_id
    )
